...
function gtaMenu:Window(windowProperty)
	...
	if windowProperty.Clickable then
		assert(typeof(windowProperty.Clickable) == "boolean", ("Clickable accepts type boolean got %s"):format(typeof(windowProperty.BackBind)))
	end
	...
	local instanceName = data.add({name = randomString(32)}, "instanceName")
	local isClickable = data.add({boolean = windowProperty.Clickable or false}, "isClickable")
	...
	if not get(isClickable.boolean) then
	...
	function tabs:Tab(tabProperty)
		...
		function components:Divider()
			return addComponent("divider", tabWindow, {})
		end
		return tabs[tabProperty.uid]
	end
	function tabs:GetName()
		return get(instanceName.name)
	end
...
accent = Color3.fromHex("#FFA411"),

selectedTab = Color3.fromHex("#FFA411"),
toggled = Color3.fromRGB(255, 164, 17),
...
return function(buttonProperty)
	...
	local isEnabled = typeof(buttonProperty.IsEnabled) == "boolean" and value(buttonProperty.IsEnabled) or value(true)
	...
	local isClickable = data.find("isClickable")
	...
	observe(selectedTab.data):onChange(function()
		local tab = get(selectedTab.data)
		if tab.uid == buttonProperty.uid then
			local index = get(currentIndex.index)
			if index == buttonProperty.index then
				isHovered:set(true)
				hoveredComponent:update("data", buttonProperty)
	...
	local buttonComponent = new "TextButton" {
		...
		[onevent "Activated"] = function()
			if get(isClickable.boolean) then
				if not get(isEnabled) then
					return
				end
				task.spawn(get(callback))
				hoveredComponent:update("data", buttonProperty)
				currentIndex:update("index", buttonProperty.index)
			end
		end,
	...
	if not get(isClickable.boolean) then
		ContextActionService:BindActionAtPriority(buttonUID, function(_, inputState)
			...
			if get(isHidden.boolean) or not get(isHovered) or get(selectedTab.data).uid ~= buttonProperty.uid or not get(isEnabled) then
				return Enum.ContextActionResult.Pass
			end
			task.spawn(get(callback))
			return Enum.ContextActionResult.Sink
...
return function(dividerProperty)
    local dividerComponent = new "Frame" {
        Parent = dividerProperty.Parent,
        ...
        [children] = {
            new "Frame" {
                ...
                BackgroundColor3 = theme.get("text2", true)
            }
        }
    }
end
...
return function(dropdownProperty)
	...
	if dropdownProperty.Regex then
		assert(typeof(dropdownProperty.Regex) == "function", ("Regex accepts type function got %s"):format(typeof(dropdownProperty.Regex)))
	end

	local name = value(dropdownProperty.Name)
	local items = value({})
	local callback = value(dropdownProperty.Callback)
	local isEnabled = typeof(dropdownProperty.IsEnabled) == "boolean" and value(dropdownProperty.IsEnabled) or value(true)
	local disabledText = value(dropdownProperty.DisabledText or "Component Disabled")
	local regex = dropdownProperty.Regex or function(itemToClean)
		return itemToClean
	end
	...
	local currentIndex = data.find(dropdownProperty.uid)
	local selectedTab = data.find("selectedTab")
	local hoveredComponent = data.find("hoveredComponent")
	...
	local searchHover = value(false)
	local searchIndex = get(totalDropdownIndex.total) + 1
	totalDropdownIndex:update("total", searchIndex)
	local searchFocus = value(false)
	local searchRef = value()
	local capturedText = value("")
	local isSearching = value(false)
	local searchListIndex = data.add({index = 1})
	local totalSearchListIndex = data.add({total = 1})
	local lastReturnedValue = value()
	...
	observe(selectedTab.data):onChange(function()
		local tab = get(selectedTab.data)
		if tab.uid == dropdownProperty.uid then
			local index = get(currentIndex.index)
			if index == dropdownProperty.index then
				isHovered:set(true)
				hoveredComponent:update("data", dropdownProperty)
	...
	new "TextButton" {
		...
		[onevent "Activated"] = function()
			hoveredComponent:update("data", dropdownProperty)
			currentIndex:update("index", dropdownProperty.index)
			task.wait()
	...
	observe(dropdownIndex.index):onChange(function()
		...
		if index == searchIndex then
			searchHover:set(true)
		else
			searchHover:set(false)
		end
	end)
	observe(searchListIndex.index):onChange(function()
		local index = get(searchListIndex.index)
		if index == searchIndex then
			searchHover:set(true)
		else
			searchHover:set(false)
	...
	if get(dropdownIndex.index) == searchIndex then
		searchHover:set(true)
	end

	observe(capturedText):onChange(function()
		searchListIndex:update("index", 1)
		totalSearchListIndex:update("total", 1)

		if get(searchListIndex.index) == searchIndex then
			searchHover:set(true)
		end

		local text = get(capturedText)
		if text ~= "" then
			isSearching:set(true)
			for _,instance in get(itemElements) do
				if get(capturedText) ~= text then
					break
				end
				if not get(isSearching) then
					break
				end
				local itemName = string.split(instance.Name, ":")[1]
				local itemData = data.find(string.split(instance.Name, ":")[2])
				if string.find(regex(itemName):lower(), text:lower()) then
					local newItemSearchIndex = get(totalSearchListIndex.total) + 1
					itemData:update("searchIndex", newItemSearchIndex)
					totalSearchListIndex:update("total", newItemSearchIndex)
					itemData:update("doShow", true)
				else
					itemData:update("doShow", false)
				end
			end
		else
	...
	new "Frame" {
		...
		[children] = {
			...
			new "TextButton" {
				...
				[onevent "Activated"] = function()
					selectedTab:update("data", get(currentIndex.data))
						...
						[children] ={
							new "TextBox" {
								...
								[onevent "Changed"] = function(changedProperty)
									local textbox = get(searchRef)
									if changedProperty == "Text" and textbox.Text and textbox.Text ~= get(capturedText) then
										capturedText:set(textbox.Text)
									end
								end,
	...
	if not get(isClickable.boolean) then
		ContextActionService:BindActionAtPriority(dropdownUID.." Open", function(_, inputState)
			...
			if get(isHidden.boolean) or not get(isHovered) or get(selectedTab.data).uid ~= dropdownProperty.uid or get(isInDropdown.boolean) or not get(isEnabled) then
				return Enum.ContextActionResult.Pass
			end
			selectedTab:update("data", currentDropdown)
			isInDropdown:update("boolean", true)
		...
		ContextActionService:BindActionAtPriority(dropdownUID.." Focus", function(_, inputState)
			if inputState ~= Enum.UserInputState.Begin then
				return Enum.ContextActionResult.Pass
			end
			if get(isHidden.boolean) or not get(searchHover) or get(selectedTab.data).uid ~= currentDropdown.uid or not get(isInDropdown.boolean) then
				return Enum.ContextActionResult.Pass
			end
			searchFocus:set(true)
			local textbox = get(searchRef)
		...
		ContextActionService:BindActionAtPriority(dropdownUID.." Up", function(_, inputState)
			if inputState == Enum.UserInputState.End and get(isHeld) and get(currentKey) == get(keys.key).up then
				isHeld:set(false)
			end
			if inputState ~= Enum.UserInputState.Begin then
				return Enum.ContextActionResult.Pass
			...
			isHeld:set(true)
			currentKey:set(get(keys.key).up)
			local rampMax = 50
			local currentRamp = 5
			while get(isHeld) and get(currentKey) == get(keys.key).up do
				if not get(isSearching) then
					...
					local current = get(searchListIndex.index)
					if get(totalSearchListIndex.total) > 0 then
						searchListIndex:update("index", math.clamp(current - 1, 1, get(totalSearchListIndex.total)))
					end
				end
		...
		ContextActionService:BindActionAtPriority(dropdownUID.." Down", function(_, inputState)
			if inputState == Enum.UserInputState.End and get(isHeld) and get(currentKey) == get(keys.key).down then
				isHeld:set(false)
			end
			if inputState ~= Enum.UserInputState.Begin then
				return Enum.ContextActionResult.Pass
			...
			isHeld:set(true)
			currentKey:set(get(keys.key).down)
			local rampMax = 50
			local currentRamp = 5
			while get(isHeld) and get(currentKey) == get(keys.key).down do
				if not get(isSearching) then
					local current = get(dropdownIndex.index)
					if get(totalDropdownIndex.total) > 0 then
						dropdownIndex:update("index", math.clamp(current + 1, 1, get(totalDropdownIndex.total)))
	...
	observe(items):onChange(function()
		...
		for _,item in get(items) do
			...
			observe(searchListIndex.index):onChange(function()
				local index = get(searchListIndex.index)
				if index == get(itemData.searchIndex) then
					itemHover:set(true)
				else
					itemHover:set(false)
				end
			...
			local itemComponent = new "TextButton" {
				Visible = computed(function()
					return get(itemData.doShow)
				end),
				Name = computed(function()
					return tostring(get(itemValue)..":"..itemData.uid)
				end),
				Interactable = isClickable.boolean,
				...
				[onevent "Activated"] = function()
					if get(capturedText) == "" then
						dropdownIndex:update("index", itemIndex)
					else
						searchListIndex:update("index", get(itemData.searchIndex))
					end
					if dropdownProperty.Multiselect then
						...
						lastReturnedValue:set(get(selectedItems))
						task.spawn(get(callback), get(selectedItems))
					else
						lastReturnedValue:set(get(itemValue))
			...
			if not get(isClickable.boolean) then
				ContextActionService:BindActionAtPriority(itemData.uid.." Return", function(_, inputState)
					if inputState ~= Enum.UserInputState.Begin then
						return Enum.ContextActionResult.Pass
					end
	...
	if get(currentIndex.index) == dropdownProperty.index then
		isHovered:set(true)
	end
	...
	function returnFunctions:SetItems(newTable) -- Sets a new table of items
		...
		selectedItems:set({})
		totalDropdownIndex:update("total", 1)
		items:set(newTable)
		dropdownIndex:update("index", math.clamp(get(dropdownIndex.index), 1, get(totalDropdownIndex.total)))
		...
		for _,v in get(items) do
			...
			if dropdownProperty.Multiselect then
				if table.find(get(lastReturnedValue), item) then
					table.insert(multiselectTable, item)
					foundAnything = true
				end
			else
				if get(lastReturnedValue) == item then
					foundAnything = true
		...
		if foundAnything then
			if dropdownProperty.Multiselect then
				selectedItems:set(multiselectTable)
				task.spawn(get(callback), get(selectedItems))
			end
		else
			if dropdownProperty.Multiselect then
				selectedItems:set({get(items)[1]})
				task.spawn(get(callback), get(selectedItems))
			else
				task.spawn(get(callback), get(items)[1])
			end
		end
	end
...
return function(keybindProperty)
	...
	local isEnabled = typeof(keybindProperty.IsEnabled) == "boolean" and value(keybindProperty.IsEnabled) or value(true)
	...
	observe(selectedTab.data):onChange(function()
		local tab = get(selectedTab.data)
		if tab.uid == keybindProperty.uid then
			local index = get(currentIndex.index)
			if index == keybindProperty.index then
				isHovered:set(true)
				hoveredComponent:update("data", keybindProperty)
				...
				[children] = {
					...
					new "TextBox" {
						Interactable = computed(function()
							...
							if (not get(isClickable.boolean)) and (not get(focused)) then
								return false
							end
							return true
						end),
						...
						[onevent "Focused"] = function()
							if get(isClickable.boolean) then
								hoveredComponent:update("data", keybindProperty)
								currentIndex:update("index", keybindProperty.index)
							end
							focused:set(true)
							task.wait()
							waiting:set(true)
	...
	local function functionForBind(_, inputState)
		if inputState ~= Enum.UserInputState.Begin or not get(isEnabled) then
			return Enum.ContextActionResult.Pass
		end
		if get(isToggle) then
	...
	if not get(isClickable.boolean) then
		ContextActionService:BindActionAtPriority(keybindUID.." Focus", function(_, inputState)
			...
			if get(isHidden.boolean) or not get(isHovered) or get(selectedTab.data).uid ~= keybindProperty.uid or not get(isEnabled) then
				return Enum.ContextActionResult.Pass
			end
			focused:set(true)
	...
	local connection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
		...
		if get(waiting) then
			...
			if not table.find(blacklistedBinds, input.KeyCode) and not table.find(get(keys.key), input.KeyCode) and not table.find(get(currentBinds.binds), input.KeyCode) then
				local binds = get(currentBinds.binds)
				table.remove(binds, table.find(binds, get(currentBind)))
				...
				if get(currentBind) ~= input.KeyCode then
...
return function(sliderProperty)
	...
	local isEnabled = typeof(sliderProperty.IsEnabled) == "boolean" and value(sliderProperty.IsEnabled) or value(true)
	local disabledText = value(sliderProperty.DisabledText or "Component Disabled")

	local previewAssetId = value(sliderProperty.PreviewAssetId or "")
	local imageSize = value(sliderProperty.ImageSize or UDim2.new(0,0,0,0))
	...
	observe(selectedTab.data):onChange(function()
		local tab = get(selectedTab.data)
		if tab.uid == sliderProperty.uid then
			local index = get(currentIndex.index)
			if index == sliderProperty.index then
				isHovered:set(true)
				hoveredComponent:update("data", sliderProperty)
		...
		[children] = {
			...
			new "Frame" { -- Slider
				...
				[children] = {
					...
					new "TextButton" {
						Interactable = computed(function()
							return get(isClickable.boolean)
						end),
						...
						[onevent "InputBegan"] = function(input)
							if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch and get(textbuttonRef) then
								hoveredComponent:update("data", sliderProperty)
								currentIndex:update("index", sliderProperty.index)
	...
	if not get(isClickable.boolean) then
		ContextActionService:BindActionAtPriority(sliderUID.." Left", function(_, inputState)
			...
			if get(isHidden.boolean) or not get(isHovered) or get(selectedTab.data).uid ~= sliderProperty.uid or not get(isEnabled) then
				return Enum.ContextActionResult.Pass
			end
			isHeld:set(true)
...
return function(tabProperty)
	...
	new "TextButton" {
		...
		[onevent "Activated"] = function()
			currentIndex:update("index", tabProperty.index)
...
return function(textboxProperty)
	...
	local isEnabled = typeof(textboxProperty.IsEnabled) == "boolean" and value(textboxProperty.IsEnabled) or value(true)
	local disabledText = value(textboxProperty.DisabledText or "Component Disabled")

	local previewAssetId = value(textboxProperty.PreviewAssetId or "")
	local imageSize = value(textboxProperty.ImageSize or UDim2.new(0,0,0,0))
	...
	observe(selectedTab.data):onChange(function()
		local tab = get(selectedTab.data)
		if tab.uid == textboxProperty.uid then
			local index = get(currentIndex.index)
			if index == textboxProperty.index then
				isHovered:set(true)
				hoveredComponent:update("data", textboxProperty)
						...
						[children] = {
							...
							new "TextBox" {
								...
								[onevent "Focused"] = function()
									...
									if get(isClickable.boolean) then
										hoveredComponent:update("data", textboxProperty)
										currentIndex:update("index", textboxProperty.index)
									end
								end,
...
return function(toggleProperty)
	...
	local isEnabled = typeof(toggleProperty.IsEnabled) == "boolean" and value(toggleProperty.IsEnabled) or value(true)
	local disabledText = value(toggleProperty.DisabledText or "Component Disabled")

	local previewAssetId = value(toggleProperty.PreviewAssetId or "")
	local imageSize = value(toggleProperty.ImageSize or UDim2.new(0,0,0,0))
	...
	observe(selectedTab.data):onChange(function()
		local tab = get(selectedTab.data)
		if tab.uid == toggleProperty.uid then
			local index = get(currentIndex.index)
			if index == toggleProperty.index then
				isHovered:set(true)
				hoveredComponent:update("data", toggleProperty)
	...
	local toggleComponent = new "TextButton" {
		...
		[onevent "Activated"] = function()
			if get(isClickable.boolean) then
				...
				hoveredComponent:update("data", toggleProperty)
				currentIndex:update("index", toggleProperty.index)
			end
		end,
	...
	if not get(isClickable.boolean) then
		ContextActionService:BindActionAtPriority(toggleUID, function(_, inputState)
			...
			if get(isHidden.boolean) or not get(isHovered) or get(selectedTab.data).uid ~= toggleProperty.uid or not get(isEnabled) then
				return Enum.ContextActionResult.Pass
			end
			isToggled:set(not get(isToggled))
			task.spawn(get(callback), get(isToggled))
    ...
    [75] = function()local wax,script,require=ImportGlobals(75)local ImportGlobals return (function(...)--[[
...
return function(target)
    ...
    local ExtractItem = function(itemToClean)
    ...
    local dropdown1 = tab:Dropdown({
        Name = "test",
        Items = {{
            Value = '<font color="rgb(255,125,125)">apple</font>',
            Image = "ric is a retard"
        },
        '<font color="rgb(255,125,125)">banana</font>',
        '<font color="rgb(255,125,125)">carrot</font>',
        '<font color="rgb(255,125,125)">dingleberry</font>',
        '<font color="rgb(255,125,125)">egg plant</font>',
        '<font color="rgb(255,125,125)">frank</font>',
        '<font color="rgb(255,125,125)">grape</font>',
        'helicopter'},
        Multiselect = true,
        Regex = ExtractItem,
        Callback = function(value)
							...
							new "Frame" {
								...
								[children] = {
									...
									new "TextLabel" {
										...
										TextColor3 = theme.get("text", true),
										Text = computed(function()
											if get(hoveredComponent.data).Description and get(isInTab.boolean) and get(selectedTab.data).uid == get(hoveredComponent.data).uid then
...
[78] = function()local wax,script,require=ImportGlobals(78)local ImportGlobals return (function(...)return {
defaultTab = Color3.fromHex("#FFA411"),
...
return function(SortOrder, FillDirection,HorizontalAlignment,VerticalAlignment,Padding)
    ...
    [83] = function()local wax,script,require=ImportGlobals(83)local ImportGlobals return (function(...)local Fusion = require(script.Parent.Parent.Bundles.Fusion)
...
return function(callback, speed, damping)
    ...
    [86] = function()local wax,script,require=ImportGlobals(86)local ImportGlobals return (function(...)local connections = {}
...
return connectionsFunction
    ...
    [87] = function()local wax,script,require=ImportGlobals(87)local ImportGlobals return (function(...)local fonts = {
...
return function(font:string, weight:Enum.FontWeight?, style:Enum.FontStyle?)
    ...
    [88] = function()local wax,script,require=ImportGlobals(88)local ImportGlobals return (function(...)local functions = {}
...
return functions
    ...
    [89] = function()local wax,script,require=ImportGlobals(89)local ImportGlobals return (function(...)return function(o,s)
	local services = require(script.Parent.Parent.Bundles.services)
    ...
    [90] = function()local wax,script,require=ImportGlobals(90)local ImportGlobals return (function(...)return function(value, dependency)
    ...
    [91] = function()local wax,script,require=ImportGlobals(91)local ImportGlobals return (function(...)return function(text:string, properties)
    text = text or ""
    ...
    [92] = function()local wax,script,require=ImportGlobals(92)local ImportGlobals return (function(...)local charset = {}
...
return randomString
    ...
    [93] = function()local wax,script,require=ImportGlobals(93)local ImportGlobals return (function(...)local references = {}
local returnFunctions = {}
